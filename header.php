...
<head>
	...
	<title>
		<?php if (function_exists('is_tag') && is_tag()) { echo 'Tag Archive for &quot;'.$tag.'&quot; - '; } elseif (is_archive()) { wp_title(''); echo ' Archive - '; } elseif (is_search()) { echo 'Search for &quot;'.esc_html($s).'&quot; - '; } elseif (!(is_404()) && (is_single()) || (is_page())) { wp_title(''); echo ' - '; } elseif (is_404()) { echo 'Not Found - '; } if (is_home()) { bloginfo('name'); echo ' - '; bloginfo('description'); } else { bloginfo('name'); } ?>
	</title>
	...
	<?php $options = get_option('respansive_options'); if (($options['themeinput'] == 'theme-default')) { echo ""; } elseif (($options['themeinput'] == 'none')) { echo ""; } else { echo "<!-- Included Respansive Theme --><link rel='stylesheet' href='".get_template_directory_uri()."/assets/css/themes/".$options['themeinput'].".css'>"; } ?>
